...
async def list_articles(
    ...
    read_ids = {r.article_id for r in db.query(ArticleRead.article_id).filter(ArticleRead.article_id.in_([a.id for a in articles])).all()}
    return [
        ArticleOut(
...
async def get_article(
    ...
    is_read = db.query(ArticleRead).filter(ArticleRead.article_id == article_id).first() is not None
...
async def mark_read(
    ...
    if not article:
        raise HTTPException(status_code=404, detail="Article not found")
    existing = db.query(ArticleRead).filter(ArticleRead.article_id == article_id).first()
    if not existing:
        db.add(ArticleRead(article_id=article_id, read_at=datetime.now(timezone.utc)))
        db.commit()
...
async def mark_unread(
    ...
    db.query(ArticleRead).filter(ArticleRead.article_id == article_id).delete()
    db.commit()
