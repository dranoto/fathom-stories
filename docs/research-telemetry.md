...
**Wired locally, not deployed:** `JevClassifier.classify()` records destination choices, exact shown-event IDs after trimming, destination confidence and Jev request metadata; `assess_duplicate()` uses a separate `jev_dedup` provider lane. The retry helper records individual grouping/regroup attempts, the summary stream records each stream attempt, and full-model dedup has a `regroup_dedup` lane. Regroup records committed article/event outcomes after dedup, and add/remove/move/merge/split record manual feedback after successful commit. All writes are opt-in through `FATHOM_RESEARCH_DB_PATH`; nothing is collected in production until a separately approved build/deployment enables it.
